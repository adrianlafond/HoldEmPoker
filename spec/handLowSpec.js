...
describe('test low hands', function () {
  ...
  it('should find the best "normal" low hand', function () {
    var hand = new Hand({ cards: ['AS', '4C', 'QH', '4D', '3D', '5H', '2C'], low: Hand.ACE_TO_FIVE_LOW })
    expect(hand.cardsLow).toEqual(['AS', '2C', '3D', '4D', '5H'])

    hand = new Hand({ cards: ['6S', '4C', 'QH', '4D', '3D', '5H', '2C'], low: Hand.ACE_TO_FIVE_LOW })
    expect(hand.cardsLow).toEqual(['2C', '3D', '4D', '5H', '6S'])

    hand = new Hand({ cards: ['7S', '4C', 'QH', '4D', '3D', '5H', '2C'], low: Hand.ACE_TO_FIVE_LOW })
    expect(hand.cardsLow).toEqual(['2C', '3D', '4D', '5H', '7S'])

    hand = new Hand({ cards: ['AS', '4C', 'AH', '4D', '3D', 'AD', '2C'], low: Hand.ACE_TO_FIVE_LOW })
  ...
  it('should find the best low hand when ignoreFlushes is false', function () {
    var cards = ['AS', '4C', '4D', '4S', '3S', '5S', '2S'],
        hand = new Hand({ cards: cards, low: Hand.ACE_TO_FIVE_LOW })
    expect(hand.cardsLow).toEqual(['AS', '2S', '3S', '4S', '5S'])

    hand = new Hand({ cards: cards, low: Hand.ACE_TO_SIX_LOW })
